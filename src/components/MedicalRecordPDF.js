...
const SOAP_COLOR = {
  soap_subjective: { letter: "S", title: "Subjetivo",               color: C.blue,   bg: C.blueBg  },
  soap_objective:  { letter: "O", title: "Objetivo",                color: C.teal,   bg: C.tealBg  },
  soap_assessment: { letter: "A", title: "Evaluación / Diagnóstico", color: C.amber,  bg: C.amberBg },
...
const s = StyleSheet.create({
  page: { fontFamily: "Helvetica", fontSize: 10, color: C.dark, backgroundColor: C.white, paddingTop: 36, paddingBottom: 52, paddingHorizontal: 40 },

  // Header
  header:         { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: C.border },
  logoBox:        { width: 40, height: 40, borderRadius: 8, backgroundColor: C.blue, alignItems: "center", justifyContent: "center", marginRight: 10 },
  ...
  rxBadge:        { flexDirection: "row", alignItems: "center", backgroundColor: C.blueBg, borderRadius: 6, paddingVertical: 4, paddingHorizontal: 10, borderWidth: 1, borderColor: "#bfdbfe" },
  ...
  dateText:       { fontSize: 9, color: C.muted, marginTop: 3, textTransform: "capitalize" },

  // Meta cards row
  metaRow:        { flexDirection: "row", gap: 8, marginBottom: 14 },
  metaCard:       { flex: 1, backgroundColor: C.bgLight, borderRadius: 8, padding: 10, borderWidth: 1, borderColor: C.border },
  metaLabel:      { fontSize: 7, fontFamily: "Helvetica-Bold", color: C.faint, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 },
  ...
  sectionLabel:   { fontSize: 7, fontFamily: "Helvetica-Bold", color: C.faint, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 6 },

  // Vitals
  vitalsGrid:     { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  vitalCard:      { width: "23%", backgroundColor: C.bgLight, borderRadius: 6, padding: 8, borderWidth: 1, borderColor: C.border, alignItems: "center" },
  vitalValue:     { fontSize: 14, fontFamily: "Helvetica-Bold", color: C.dark, marginBottom: 1 },
  vitalUnit:      { fontSize: 7, color: C.faint },
  vitalLabel:     { fontSize: 7, color: C.muted, marginTop: 2 },

  // SOAP
  soapCard:       { borderRadius: 8, marginBottom: 8, overflow: "hidden", borderWidth: 1, borderColor: C.border },
  soapHeader:     { flexDirection: "row", alignItems: "center", paddingVertical: 7, paddingHorizontal: 10 },
  ...
  soapBody:       { backgroundColor: C.white, paddingVertical: 10, paddingHorizontal: 12 },
  soapText:       { fontSize: 9.5, color: C.mid, lineHeight: 1.6 },

  // Medications / notes
  contentCard:    { backgroundColor: C.white, borderRadius: 8, padding: 12, borderWidth: 1, borderColor: C.border, marginBottom: 10 },
  contentText:    { fontSize: 9.5, color: C.mid, lineHeight: 1.6 },
  noteCard:       { backgroundColor: C.amberBg, borderRadius: 8, padding: 12, borderWidth: 1, borderColor: "#fde68a", marginBottom: 10 },
...
function fmtDate(iso, opts = { weekday: "long", day: "numeric", month: "long", year: "numeric" }) {
  if (!iso) return "—";
...
function MedicalRecordDocument({ record, organization, config }) {
  ...
  const hasVitals = VITALS_META.some((v) => record?.[v.field] != null && record?.[v.field] !== "");
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.metaRow}>
          <View style={s.metaCard}>
            <Text style={s.metaLabel}>Doctor / Profesional</Text>
              ...
              return (
                <View key={field} style={s.soapCard}>
                  <View style={[s.soapHeader, { backgroundColor: cfg.bg }]}>
                    <Text style={[s.soapLetter, { color: cfg.color }]}>{cfg.letter}</Text>
        ...
        <View style={s.footer} fixed>
          <Text style={s.footerText}>
            Generado el {fmtDate(new Date().toISOString(), { day: "numeric", month: "long", year: "numeric" })} · {organization?.name}
          </Text>
...
export default function DownloadMedicalRecordPDF({ record, organization, config }) {
  ...
  return (
    <PDFDownloadLink
      document={<MedicalRecordDocument record={record} organization={organization} config={config} />}
      fileName={fileName}
    >
      {({ loading }) => (
        <button
