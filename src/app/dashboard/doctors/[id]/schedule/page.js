...
export default function DoctorSchedulePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((row) => {
          const existing = doc.schedules?.find(
            (s) => s.day_of_week === row.key,
          );
          if (existing) {
            return {
              ...row,
  ...
  const updateRow = (index, field, value) => {
    setRows((prev) =>
      prev.map((r, i) =>
        i === index ? { ...r, [field]: value, dirty: true } : r,
      ),
    );
  };
  ...
  const toggleRow = (index) => {
    setRows((prev) =>
      prev.map((r, i) =>
        i === index ? { ...r, active: !r.active, dirty: true } : r,
      ),
    );
  };
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      await Promise.all(
        dirty.map((row) => {
          if (row.active) {
            if (row.scheduleId) {
              return api.patch(
                `/api/v1/doctors/${id}/schedules/${row.scheduleId}`,
                {
                  schedule: {
      ...
      const msg =
        err.response?.data?.errors?.join(", ") || "Error al guardar el horario";
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const handleAddBlock = async () => {
    ...
    if (
      new Date(blockForm.end_datetime) <= new Date(blockForm.start_datetime)
    ) {
    ...
    try {
      ...
      const msg =
        err.response?.data?.errors?.join(", ") || "Error al agregar bloqueo";
      toast.error(msg);
    } finally {
  ...
  const totalSlots = rows
    ...
    .reduce(
      (acc, r) =>
        acc + countSlots(r.start, r.end, doctor?.consultation_duration || 30),
      0,
    );
          ...
          <div>
            ...
            {doctor && (
              <p className="text-sm mt-0.5" style={{ color: "#64748b" }}>
                ...
                {doctor.consultation_duration} min por cita
              </p>
            )}
          </div>
        </div>
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor: row.active ? "#2563eb" : "#e2e8f0",
                    }}
                  />
                  <div
      ...
      >
        ...
        >
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "#94a3b8" }}>
              Vacaciones, almuerzos, eventos — los slots dentro de estos rangos
              no aparecerán disponibles
            </p>
          </div>
          <button
            onClick={() => {
        ...
        {blocks.length === 0 ? (
          <div className="px-6 py-10 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: "#cbd5e1" }}>
              El {config.staffLabel} está disponible en todos sus horarios
              configurados
            </p>
          </div>
        ) : (
            ...
            <tbody>
              {blocks.map((block, i) => (
                <tr
                  ...
                  style={{
                    borderBottom:
                      i < blocks.length - 1 ? "1px solid #f8fafc" : "none",
                  }}
                  onMouseEnter={(e) =>
                ...
                >
                  ...
                  <td className="px-6 py-3">
                    <p
                      className="text-sm"
                      style={{ color: block.reason ? "#0f172a" : "#cbd5e1" }}
                    >
                      {block.reason || "—"}
                    </p>
                  </td>
