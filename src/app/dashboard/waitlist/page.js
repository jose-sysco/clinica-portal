...
function AddWaitlistModal({ onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.doctor_id || !form.patient_id || !form.owner_id) {
      setError(
        `${config.staffSingularLabel || "Doctor"} y paciente son obligatorios.`,
      );
      return;
    }
    setSaving(true);
  ...
  const inputCls =
    "w-full text-sm rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500";
  const inputStyle = {
      ...
      >
        ...
        >
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "#64748b" }}>
              El staff será notificado cuando se libere un espacio.
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-lg transition-colors hover:bg-slate-100"
            style={{ color: "#94a3b8" }}
          >
            ×
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {loadingData ? (
            ...
            <>
              <div>
                <label className={labelCls} style={{ color: "#64748b" }}>
                ...
                >
                  <option value="">
                    Seleccionar {config.staffSingularLabel || "doctor"}...
                  </option>
                  {doctors.map((d) => (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="Motivo, disponibilidad horaria..."
                  className={inputCls}
                  style={inputStyle}
                />
              </div>
...
export default function WaitlistPage() {
  ...
  const handleStatusChange = async (entry, newStatus) => {
    ...
    try {
      await api.patch(`/api/v1/waitlist_entries/${entry.id}`, {
        waitlist_entry: { status: newStatus },
      });
      toast.success(
        `${entry.patient?.name} marcado como ${STATUS_LABEL[newStatus] || newStatus}`,
      );
      fetchEntries();
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Agendados",
            value: entries.filter((e) => e.status === "booked").length,
            color: "#16a34a",
            bg: "#f0fdf4",
          },
          {
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <option value="">Todos los {config.staffLabel || "doctores"}</option>
          {doctors.map((d) => (
            <option key={d.id} value={d.id}>
              {d.full_name}
            </option>
      ...
      {loading ? (
        ...
        >
          <EmptyState
            ...
            description={`Agrega pacientes aquí cuando un ${config.staffSingularLabel || "doctor"} no tenga disponibilidad inmediata.`}
                ...
                >
                  ...
                  >
                    {entry.preferred_date ? (
                      fmtDate(entry.preferred_date)
                    ) : (
                      <span style={{ color: "#94a3b8" }}>Cualquier día</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    ...
                    {entry.notified_at && (
                      <p
                        ...
                        style={{ color: "#d97706" }}
                      >
                        Notif. {fmtTime(entry.notified_at)}
                      </p>
                    )}
                  </td>
                  <td className="px-4 py-4">
